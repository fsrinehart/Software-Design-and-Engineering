 ...
 * Language: Java
 * Purpose: This class creates the prompts for the medical application
 *
 */
...
public class MedicalRecordPrompt {
	...
	public void addTreatment(Scanner scanner, String patientId) {
		...
		while (input != 0) {
			...
			// ADD: input of doctor attending
			System.out.println("Enter the attending doctor:");
			...
			// ADD: doctor to new Treatment model
			Treatment treatment = new Treatment(doctor, treatmentDate, diagnose, description);
			MedicalRecord medicalRecord = MedicalRecordService.getReference().getMedicalRecord(patientId);
	...
	public void addMedication(Scanner scanner, String patientId) {
		...
		while (input != 0) {
			...
			// Allows the user to Input the doctors name
			System.out.println("Enter doctor:");
			String doctor = scanner.next();
			
			// Medication class is fixed which allows doctor input
			Medication medication = new Medication(name, startDate, endDate, dose, doctor);
			MedicalRecord medicalRecord = MedicalRecordService.getReference().getMedicalRecord(patientId);
